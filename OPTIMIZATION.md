...
旧的 `SM3Digest` 先把所有数据通过 `update()` 拷贝到内部 `msgBuf`，`doFinal()` 时再计算 padding 长度、扩容、再次拷贝，最后才调用压缩函数。对 1MB 数据来说，每次 hash 都会触发一次 ~1MB 的数组分配和拷贝，GC 与内存带宽被白白消耗。

#### 修复方案

改为 **streaming update**：
- 内部只保留 64 字节缓冲。
- `update()` 时直接把完整 64 字节分组喂给压缩函数，只剩不足 64 字节的尾部缓存。
- `doFinal()` 最多分配 128 字节做最终填充。

公开 API 完全不变：`update(byte[])`、`update(byte[], int, int)`、`doFinal()`、`doFinal(byte[])` 行为一致。

#### 效果

| 项目 | JDK25 基线 | JDK25 优化后 | JDK8 优化后 |
|---|---|---|---|
| SM3 (1MB) | 3339 ms | **2996 ms** | **3054 ms** |
| 提升 | - | **-10.3%** | **-8.5%** |

#### 已移除的无收益改动

此前尝试的以下优化方向经测试没有实质收益或反而负优化，已回退：
- MRJAR 多版本结构（`src/main/java-9/21/25`）
- JDK 9+ 直接 VarHandle
- JDK 21 Virtual Threads SM4 CTR
- JDK 25 Vector API SM3
- JNI SM3 CF（反而慢 35%+）

项目回归普通单版本 Java 8 构建，降低维护复杂度。
