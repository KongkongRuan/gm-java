...
## 3.2.6 更新内容
- **SM2 native 验签新增热公钥 comb 路径（自适应升级，阈值 4）**：线程本地公钥缓存对同一公钥连续验签计数，命中 `GM_COMB_PROMOTE_THRESHOLD`（默认 4）时一次性构建 255 项 comb 预计算表（约 16 KB 线程本地存储，构建约 100 µs），后续验签改用两次 comb 标量乘（各 32 次倍点 + 至多 32 次加点）加一次 `jac_add`，替代交织 wNAF 的 256 次倍点；冷公钥与多公钥轮换保持原 wNAF 路径，换 key 计数器归零、永不触发升级。native 同 key 热路径验签 47.3 µs → 18.6 µs（省约 61%）；首次调用/强制未命中/2–100 公钥轮换相对 wNAF 路径回退 ≤1%；comb 冷路径（含建表）约 121 µs vs wNAF 冷路径约 58 µs，同一公钥第 2 次验签起回本；Java 端到端验签中位约 19.2 µs（BC 约 46.4 µs）。`native/native_mul_test.c` 新增 comb/自适应路由/wNAF oracle 三方差分（各 10 万有效 + 10 万篡改签名）、1 万组点级差分（含 25 组标量边界）、14 组验签边界用例，以及升级逻辑测试（阈值处精确切换、交替公钥不升级）
## 3.2.5 更新内容
