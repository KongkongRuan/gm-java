...
/**
 * SM3 with Unsafe-backed direct-buffer friendly input path.
 * Public API identical to {@link SM3Digest}; additionally supports
 * {@link #doFinal(ByteBuffer)} to avoid copying direct buffers.
 */
public class SM3DigestDirect {
    ...
    private static int readIntBE(byte[] b, int off) {
        if (UNSAFE != null) {
            return Integer.reverseBytes(UNSAFE.getInt(b, BYTE_ARRAY_BASE_OFFSET + off));
        }
        return bytesToIntBE(b, off);
    }
    ...
    private static void CF(int[] V, byte[] padded, int offset, int[] out, int[] W, int[] W1) {
        for (int i = 0; i < 16; i++) {
            W[i] = readIntBE(padded, offset + i * 4);
        }
        for (int j = 16; j < 68; j++) {
    ...
    /**
     * Process a direct {@link ByteBuffer} without copying its payload to a heap array.
     * The buffer's position is advanced by the consumed bytes.
     */
    public void update(ByteBuffer buf) {
        int len = buf.remaining();
        if (len <= 0) return;

        if (buf.isDirect() && UNSAFE != null) {
            ...
            if (bufferLen > 0) {
                ...
                for (int i = 0; i < copy; i++) {
                    buffer[bufferLen + i] = UNSAFE.getByte(addr + pos + i);
                }
                bufferLen += copy;
                pos += copy;
                len -= copy;
            ...
            while (len >= 64) {
                // Copy one block from native memory to a temporary heap block
                // because the SM3 CF works on byte[]. A future native CF could
                // consume the direct address directly.
                byte[] block = new byte[64];
                for (int i = 0; i < 64; i++) {
                    block[i] = UNSAFE.getByte(addr + pos + i);
                }
            ...
            if (len > 0) {
                for (int i = 0; i < len; i++) {
                    buffer[i] = UNSAFE.getByte(addr + pos + i);
                }
                bufferLen = len;
            }
            buf.position(buf.position() + (pos - buf.position()));
        } else {
            int lim = buf.limit();
            byte[] arr;
            int off;
    ...
    public byte[] doFinal(ByteBuffer buf) {
        resetState();
        update(buf);
        return doFinal();
    }
